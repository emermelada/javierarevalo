...
(static output, no client framework), hand-written CSS and ~15 lines of vanilla JS.
...
- `src/styles/global.css`: the entire design system, token-driven, organised
  ...
  over adding an override. Design language is **research paper**: white page,
  IBM Plex Sans for structure, Source Serif 4 for reading, one Nordic blue
  accent (`--accent`). Home sections are full-width bands (every other one
  tinted, the award band dark), each with its title in a left margin column. Audience it is written for: admissions committees and engineers.
- `src/layouts/Base.astro`: `<head>`, sticky header, contact footer, and the
  only JavaScript (copy-email, degrades to `mailto:`).
...
- `src/data/cv.ts`: **the CV content itself** (profile, pitch, about, award,
...
- `src/pages/work/[slug].astro`: case-study template. Its media slot renders,
  ...
  Phone captures sit side by side; any landscape capture stacks full width.

## Conventions

- **No scroll-triggered motion.** Hover and focus states only. The scroll
  reveal of the previous design was removed on purpose; don't bring it back.
...
- New project = new markdown file in `src/content/work/` with the next `order`
  value; the home list and prev/next case navigation pick it up automatically.
  `cv: false` keeps a project off the one-page PDF (the site still lists it);
  that is the lever when the sheet spills onto a second page.
